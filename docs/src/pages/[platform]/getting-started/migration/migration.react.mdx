...
Migration guides for [`Liveness`](#`@aws-amplify/ui-react-liveness`), [`In-App Messaging`](#`@aws-amplify/ui-react-notifications`) and [`Storage`](/getting-started/migration#`@aws-amplify/ui-react-storage`) packages can be found towards the bottom of this page.
</Message>
